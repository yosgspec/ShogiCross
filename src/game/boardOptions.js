...
export const boardOptions = new Map([
	["default", {
		name: "--ゲームを選択--",
		playBoard: "クロス14x14",
		useStand: true,
	...
	...Object.entries(gameSoft).map(([key, option])=>{
		const xLen = boards[option.playBoard].field[0].length;
		return [key, {...option, ...boardTemplate(xLen)}];
	}),
]);
