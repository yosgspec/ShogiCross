export default `

<h3>WebApp</h3>
<div><a href="#app">ShogiCross App</a></div>
<div><a href="#dist">単品ゲーム集</a></div>
<div><a href="#playBoard">将棋類紹介</a></div>
<div><a href="#pieceViewer">将棋駒Viewer</a></div>
<div><a href="#pieceMakeImage">将棋駒画像生成</a></div>
<div><a href="#shogiVsChess">将棋対チェススレ 棋譜再生</a></div>
<div><a href="#cpuTest">CPU実装テスト</a></div>
<div><a href="#cpuTest4">CPU実装テスト(4人用)</a></div>
<div><a href="#onlineTest">オンラインテスト</a></div>

<h3>ペーパークラフト</h3>
<div><a href="https://github.com/yosgspec/ShogiCross/blob/main/paper/README.md">ペーパークラフト(GitHub)</a></div>
<div><a href="./paper.zip">ペーパークラフトDL(ZIP)</a></div>
<div><a href="#doc/chogi">超棋(過去制作物)</a></div>

<h3>ライブラリ</h3>
<div><a href="#doc/getStarted">はじめに(ダウンロード)</a></div>
<div><a href="https://github.com/yosgspec/ShogiCross">GitHub</a></div>
<div><a href="./doc/api/">APIドキュメント</a></div>
<div><a href="https://github.com/yosgspec/ShogiCross/tree/main/doc/pieces">駒一覧</a></div>

<h3>ボード設定</h3>
<div><a href="#doc/board/board">ボードを構成する</a></div>
<div><a href="#doc/boardOption/gameSoft">ゲームをプリセットから起動</a></div>
<div><a href="#doc/boardOption/playBoard">ボードタイプの変更</a></div>
<div><a href="#doc/boardOption/useStand">駒台の使用有無</a></div>
<div><a href="#doc/boardOption/playersOption">プレイヤー設定/駒の初期配置</a></div>
<div><a href="#doc/boardOption/players">ゲームの参加人数</a></div>
<div><a href="#doc/boardOption/canvasFit">Canvasの寸法調整</a></div>
<div><a href="#doc/boardOption/boardSizes">ボードの寸法を変更</a></div>
<div><a href="#doc/boardOption/backgroundColor">背景色を変更</a></div>
<div><a href="#doc/boardOption/useRankSize">駒の格で大きさを変更しない</a></div>
<div><a href="#doc/boardOption/isDrawShadow">駒の影の描写を変更</a></div>
<div><a href="#doc/boardOption/autoDrawing">描写の自動更新を変更</a></div>
<div><a href="#doc/boardOption/moveModeFree">自由移動の有効化</a></div>
<div><a href="#doc/boardOption/moveModeViewOnly">移動を無効化</a></div>
<div><a href="#doc/board/onDrawed">描写時に任意の処理を行う</a></div>
<div><a href="#doc/board/onGameOver">プレイヤーが敗北した時の処理を行う</a></div>
<div><a href="#doc/boardOption/useUIControl">操作パネル表示を変更する</a></div>
<div><a href="#doc/board/makeUIControl">操作パネル要素を取得</a></div>
<div><a href="#doc/board/downloadImage">スクリーンショットを保存する</a></div>
<div><a href="#doc/board/rotate">盤面を回転する</a></div>
<div><a href="#doc/extendData">ゲームデータを拡張する</a></div>
<div><a href="#doc/board/close">ゲームを終了する</a></div>

<h3>駒の操作</h3>
<div><a href="#doc/board/putNewPiece">駒を任意のマスに配置する</a></div>
<div><a href="#doc/board/putStartPieces">駒の初期配置(メソッド)</a></div>
<div><a href="#doc/board/setTextPieces">駒をテキスト形式で配置する</a></div>
<div><a href="#doc/board/getTextPieces">駒の配置をテキスト形式で取得</a></div>
<div><a href="#doc/record/getText">局面の記録を文字列に変換</a></div>

<h3>棋譜の操作</h3>
<div><a href="#doc/record/getTextAll">棋譜の取得</a></div>
<div><a href="#doc/record/getJson">棋譜データを取得</a></div>
<div><a href="#doc/record/setJson">棋譜データを反映</a></div>
<div><a href="#doc/record/switch">手を進める・戻す</a></div>
<div><a href="#doc/record/move">手数を移動する</a></div>

<h3>ゲームデータ仕様</h3>
<div><a href="#md/data/gameSoft">ゲーム用プリセット</a></div>
<div><a href="#md/data/games">ゲーム種類・駒配置</a></div>
<div><a href="#md/data/canvasFont">Canvas用フォント</a></div>
<div><a href="#md/data/boards">ボードの構成</a></div>
<div><a href="#md/data/panels">ボードのマス目</a></div>
<div><a href="#md/data/pieces">駒の定義</a></div>
<div><a href="#md/data/pieceRange">駒の移動範囲</a></div>
<div><a href="#md/data/pieceCost">駒の価値情報</a></div>
<div><a href="https://github.com/yosgspec/ShogiCross/blob/main/doc/data/README.md">ゲームデータ仕様(GitHub)</a></div>

<h3>連絡先</h3>
<div><a href="http://yosgspec.blog103.fc2.com">ブログ</a></div>
<div><a href="https://misskey.io/@yosgspec">Misskey</a></div>
<div><a href="https://twitter.com/yosgspec">Twitter</a></div>
<div><a href="https://potofu.me/yosgspec">POTOFU</a></div>
`;
