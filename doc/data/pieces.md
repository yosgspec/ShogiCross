...
(砲はシャンチー、チャンギのみに採用される駒を飛び越える駒の兵種)
...
|:----|:----|:----|:----|:----|
|king|王|全|王駒。この属性を持つ駒が存在しなくなると負けとなる。また、捕縛されない。|実装済|
...
|pao|炮制限|オリジナル|初回移動時に初期位置かつ同等の価値を超える駒を取ることはできない。|実装済|
|po|包|チャンギ|包で包を越えること・取ることはできない。|実装済|
|swapHorseElephant|馬象交換|チャンギ|ゲームの開始時、馬駒と象駒の位置を入れ替えることができる。|部分的|
