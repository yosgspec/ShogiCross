...
## やる予定のこと
* CPUの高速化
  * AIに任せて行ってもらったものの挙動か怪しいのでcpuCoreリポジトリに隔離中
* [ ] オンライン対戦の設定拡張
  * [ ] モード設定を有効化する
  * [ ] 対戦モード以外の場合、動かす手番を限定させないようにする
  * [ ] 手番の前後ボタンを表示し、対戦相手と同期させる
* [ ] ボードに数字を付けられるようにする(将棋/チェス/シャンチー形式)
  * data/boardsに形式は指定できるようにする。未指定で表示しない。
  * ボードオプションでも表示非表示変えられるようにする。
  * マークルックは列を上部にタイ語で表記
...
* [x] テスト追加検討
  ...
  * [x] テスト実装
    * 全部AIに任せる予定
* [x] レコード読み込み時の持ち駒が反映されないバグ対応
* [x] 初ターンからCPU動作を可能にする
* [x] ゲームを閉じた場合にCPUが自動で動き続けないようにする
* メソッド名変更
