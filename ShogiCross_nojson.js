...
function ge(p) {
  Object.assign(O, p.canvasFont), Object.assign(ue, p.gameSoft), Object.assign(q, p.games), Object.assign(J, p.boards), Object.assign(T, p.panels), Object.assign(D, p.pieces), Object.assign(ne, p.pieceRange), Object.assign(K, p.pieceCost);
}
ge(he);
function fe(p) {
...
class I {
  ...
  dropPiece(e, t = {}) {
    const { board: a } = this;
    if (!(e instanceof j) || a.moveMode === "viewOnly" || e.hasAttr("keepOut")) return;
...
function P(p, e, t, a) {
  ...
  function A(u, [g, { isAttack: m }], { oX: B, oY: E, isOwn: y, offsetX: C, offsetY: x }) {
    if (!(!y && !c(!1, t + C, a + x)))
      for (const [b, { jmps: k = 0, moves: v = 0 } = {}] of re) {
        const X = !v || v === 0;
        for (let M = E - 1; M <= E + 1; M++)
          for (let L = B - 1; L <= B + 1; L++) {
            if (u[M][L] !== b || L === B && M === E) continue;
            let _ = k || 0, H = v || 0;
            const [Se, me] = [L - B, M - E];
            for (let ee = t, te = a; ; ) {
              ee += Se, te += me;
              const R = ee + C, F = te + x;
              if (!o(R, F) || !X && H === 0) break;
              const U = _ === 0;
              U && c(m, R, F, g, U) ? (H--, h(g, R, F)) : k < 1 && H--;
              const ae = s[F][R];
      ...
      for (const E of B) {
        ...
        if (!y.piece)
          for (const C in y) {
            ...
            if (!k || !se[C] || e.isMoved && ["start", "castling"].includes(b)) continue;
            const v = [b, se[C]];
            for (const X of ke(k))
              f(k, v, X), A(k, v, X);
          }
  ...
  constructor(e, t) {
    ...
    this.playerLen = l, this.left = d, this.top = c, this.panelWidth = S, this.panelHeight = h, this.borderWidth = g, this.pieceSize = f, this.canvasBackgroundColor = m, this.field = this.field.map(
      (y, C) => [...y].map((x, b) => {
        ...
        return new j(null, x, k, v, S, h, b, C, g);
      })
    ...
    this.width = this.panelWidth * (this.xLen + 1), this.height = this.panelHeight * (this.yLen + 1), this.right = d + this.width, this.bottom = c + this.height, this.stand = new I(this), this.moveMode = E, this.record = new we(this), this.enPassant = new V();
  }
  /** ゲームを実行する
   * @param {HTMLCanvasElement} canvas - Canvas要素
  ...
  const { canvas: s } = p, n = async (c, S, h = () => {
    ...
    e = !0, await n(
      ...
      (S, h, f) => {
        ...
        A && S.checkRangeMouse(h, f) && (c.preventDefault(), A.isSelected = !0, a = S, P(p, A, u, g));
      },
      (S, h) => {
...
class Q extends Z {
  ...
  constructor(e, t) {
    ...
    let u = null, g = null;
    if (!r) {
      u = O.importAsync(), g = $.importAsync(), this.canvas = e, this.ctx = e.getContext("2d"), this.ctx.clearRect(0, 0, e.width, e.height), this.overlay = new Le(this.canvas, l), this.#a = new Xe();
    ...
    }), g.then(() => this.draw()), this.draw()), this.isGameEnd = !1, this.onDrawed = S, this.onTurnEnd = h, this.onGameOver = f, this.onGameEnd = A, r || (this.#e = Pe(this)), d && (this.#t = Array.isArray(d) ? this.makeUIControl(d, c) : this.makeUIControl(null, c), this.#t.add());
  }
  /** 操作パネルを構築
  ...
  constructor(e, t) {
    ...
    class o extends I {
      ...
      dropPiece(d, c = {}, S = !1) {
        ...
        if (!(d instanceof j) || !h.isReadyOnline || u.deg !== h.displayDeg || u.isLocal && f !== 0) return;
