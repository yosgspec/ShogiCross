...
const D = {
  ...
  async importAsync() {
    if (!this.imported)
      return Promise.all(
        de().map(async (p) => {
          this.images[p] = await le(p);
        })
      ).then((p) => this.imported = !0);
  }
}, ce = (p) => "image/" + p.replace("jpg", "jpeg");
async function pe(p, e = "image", t = "png", a = "base64") {
...
class C {
  ...
  * @type {Object<string, number>}
  ...
  constructor(e, t, a = {}) {
    ...
    try {
      Object.entries(this.range).forEach(([c, d]) => {
        Array.isArray(d) || (this.range[c] = te[d].map((S) => [...S]));
      });
    } catch (c) {
  ...
  async draw() {
    if (!this.ctx) return;
    const e = "#FF000055";
    this.imgSrc && D.imported ? (this.drawImage(), this.isSelected && this.drawMaskImage(e)) : (this.drawPiece(), this.isSelected && this.drawMask(e));
  }
  /** 駒画像を描写 */
  drawImage() {
    if (!this.ctx) return;
    const { ctx: e, size: t, deg: a } = this, i = this.imgSrc[a][this.displayPtn] ?? this.imgSrc[0][this.displayPtn], s = D.images[i];
    if (!s) return;
...
class ue {
  ...
  constructor(e, t, a, i, s, n, r, o, l) {
    Object.assign(this, U[t]), this.ctx = e, this.center = a, this.middle = i, this.width = s, this.height = n, this.left = a - s / 2, this.top = i - n / 2, this.right = a + s / 2, this.bottom = i + n / 2, this.pX = r, this.pY = o, this.borderWidth = l, this.selectColor ??= "#FF000066", this.targetColor ??= "#00FF0066", this.attr ??= [], this.piece = null, this.isSelected = !1, this.clearTarget();
  }
  /** マス目の選択状態
   * @param {boolean} value
   */
  ...
  draw() {
    ...
    this.imgSrc && D.imported ? this.drawImage() : this.drawPanel(), this.isSelected && this.drawMask(e), this.isTarget && this.drawMask(t), this.piece?.draw();
  }
  /** マス目画像を描写 */
  drawImage() {
    if (!this.ctx) return;
    const { ctx: e } = this, t = this.imgSrc, a = D.images[t];
...
function $(p, e, t, a) {
  ...
  function f(m, [h, { isAttack: B }], { oX: W, oY: w, isOwn: x }) {
    if (x)
      for (const [X, { child: b = [] } = {}] of fe)
        for (let E = 0; E < m.length; E++)
          for (let v = 0; v < m[E].length; v++) {
            const [O, L] = [v + t - W, E + a - w];
            !o(O, L) || !d(B, O, L, h) || m[E][v] !== X || S(m, b, !1, W, w) || u(h, O, L);
          }
  }
  function A(m, [h, { isAttack: B }], { oX: W, oY: w, isOwn: x, offsetX: X, offsetY: b }) {
    if (!(!x && !d(!1, t + X, a + b)))
      for (const [E, { jmps: v = 0, moves: O = 0 } = {}] of ae) {
        const L = !O || O === 0;
        for (let j = w - 1; j <= w + 1; j++)
          for (let F = W - 1; F <= W + 1; F++) {
            if (m[j][F] !== E || F === W && j === w) continue;
            let H = v || 0, R = O || 0;
            const [G, Y] = [F - W, j - w];
            for (let g = t, k = a; ; ) {
              g += G, k += Y;
              const N = g + X, M = k + b;
              if (!o(N, M) || !L && R === 0) break;
              const T = H === 0;
              T && d(B, N, M, h, T) ? (R--, u(h, N, M)) : v < 1 && R--;
              const z = s[M][N];
              if (z.piece && (H--, T || l(z)))
                break;
            }
  ...
  const { canvas: s } = p, n = async (d, S, u = () => {
    ...
    e = !0, await n(
      ...
      (S, u, f) => {
        ...
        A && S.checkRangeMouse(u, f) && (d.preventDefault(), A.isSelected = !0, a = S, $(p, A, m, h));
      },
      (S, u) => {
...
class _ {
  ...
  /** 駒台を初期化にする */
  clear() {
    this.stocks = new Map(_.#t.map((e) => [e, []]));
  }
  /** 持ち駒からボード上に配置する
...
const Ae = Object.keys(C.degChars), ee = () => ({
  panel: null,
  piece: null
});
class Ce {
  constructor() {
    this.degs = {}, Ae.forEach((e) => this.degs[e] = ee());
  }
  /** アンパッサン情報をクリア
...
class Z {
  ...
  constructor(e, t) {
    const {
      ...
      moveMode: O = "normal",
      usePlayerControl: L = !b,
      onDrawed: j = (g) => {
      },
      onTurnEnd: F = (g, k) => {
      },
      onGameOver: H = (g, k) => alert(`プレイヤー${k + 1}の敗北です。`),
      onGameEnd: R = (g, k) => g.addRecord({ end: `対戦終了 勝者${[...g.players.values()][k].degChar}` })
    } = t;
    this.#a = t, this.isHeadless = b, this.name = a, this.variant = i, this.url = s, this.desc = n, this.ctx = null, this.canvas = null;
    let G = null, Y = null;
    if (b || (G = P.importAsync(), Y = D.importAsync(), this.canvas = e, this.ctx = e.getContext("2d"), this.ctx.clearRect(0, 0, e.width, e.height), this.#i = new me(), this.#n = new we(this.canvas, v)), this.pieces = C.getPieces(this.ctx, {
      size: B,
      useRankSize: W,
      isDrawShadow: w
    ...
    this.playerLen = l, this.left = f, this.top = A, this.panelWidth = m, this.panelHeight = h, this.borderWidth = x, this.pieceSize = B, this.canvasBackgroundColor = X, this.field = this.field.map(
      (g, k) => [...g].map((N, M) => {
        const T = f + m * (M + 1), z = A + h * (k + 1);
        return new ue(this.ctx, N, T, z, m, h, M, k, x);
      })
    ...
    for (let g = 0; g < l; g++) {
      ...
      if (N.cpu = new xe(this, N), this.players.set(k, N), !!N.gameName)
        try {
    ...
    if (this.width = this.panelWidth * (this.xLen + 1), this.height = this.panelHeight * (this.yLen + 1), this.right = f + this.width, this.bottom = A + this.height, this.stand = new _(this), !b) {
    ...
    }), Y.then(() => this.draw()), this.draw()), this.isGameEnd = !1, this.onDrawed = j, this.onTurnEnd = F, this.onGameOver = H, this.onGameEnd = R, this.moveMode = O, this.record = [], this.turn = 0, b || (this.#t = ye(this)), L && (this.#e = this.makePlayerControl(), this.#e.add()), this.enPassant = new Ce();
  }
  /** 操作パネルを構築
   * @param {string[]} compList - 表示するコントロールの一覧
   ...
   */
  makePlayerControl(e) {
    return this.#e = new ne(this, e), this.#e;
  }
  /** ボードを閉じる */
  close() {
...
`), i + this.field.map(
  ...
  async downloadImage(e, t, a) {
    await pe(this.canvas, e ?? this.name ?? "shogicross", t, a);
  }
  /** オーバーレイの表示を開始します。 */
  startOverlay() {
  ...
  async playTurn() {
    ...
    e.field.flat().forEach((r) => {
      if (r.piece && r.piece.deg === t.deg) {
        const o = r, l = $(e, o.piece, o.pX, o.pY);
  ...
  async minimax(e, t, a, i, s) {
    ...
    e.field.flat().forEach((l) => {
      if (l.piece && l.piece.deg === n.deg) {
        const c = l, d = $(e, c.piece, c.pX, c.pY);
...
class xe extends I {
  ...
  constructor(e, t) {
    ...
    if (this.engine = a == null ? new I(e, t) : new K[a](e, t), !this.engine) throw new Error(`Engine "${a}" not found.`);
  }
  /** 手番操作 */
  async playTurn() {
...
function be(p) {
  Object.assign(P, p.canvasFont), Object.assign(re, p.gameSoft), Object.assign(Q, p.games), Object.assign(q, p.boards), Object.assign(U, p.panels), Object.assign(J, p.pieces), Object.assign(te, p.pieceRange), Object.assign(V, p.pieceCost);
}
be(oe);
const ke = () => [
  .../* @__PURE__ */ new Set([
    ...ne.buttonTexts + Object.values(U).map(({ displayText: p }) => p).join("") + Object.values(J).map(({ display: p }) => p ? p.join("") : "").join("")
  ])
].sort().join("");
Object.assign(P, {
  /** 読み込み済みであるか? */
  ...
  async importAsync() {
    ...
    const p = "https://fonts.googleapis.com/css2?family=", e = ke(), t = Date.now().toString();
