...
function ce() {
  return [...new Set(
    Object.values(z).flatMap(({ imgSrc: p }) => p ?? []).concat(Object.values(U).flatMap(({ imgSrc: p }) => p != null ? Object.values(p) : []).flat())
  )];
}
const D = {
  /** 読み込み済みであるか? */
  ...
  async importAsync() {
    if (!this.imported)
      return Promise.all(
        ce().map(async (p) => {
          this.images[p] = await de(p);
        })
      ).then((p) => this.imported = !0);
  }
}, pe = (p) => "image/" + p.replace("jpg", "jpeg");
async function Se(p, e = "image", t = "png", a = "base64") {
  const n = pe(t), s = document.createElement("a");
  let i;
...
class C {
  ...
  * @type {Object<string, number>}
  ...
  drawImage() {
    if (!this.ctx) return;
    const { ctx: e, size: t, deg: a } = this, n = this.imgSrc[a][this.displayPtn] ?? this.imgSrc[0][this.displayPtn], s = D.images[n];
    if (!s) return;
...
class he {
  ...
  constructor(e, t, a, n, s, i, r, o, l) {
    Object.assign(this, z[t]), this.ctx = e, this.center = a, this.middle = n, this.width = s, this.height = i, this.left = a - s / 2, this.top = n - i / 2, this.right = a + s / 2, this.bottom = n + i / 2, this.pX = r, this.pY = o, this.borderWidth = l, this.selectColor ??= "#FF000066", this.targetColor ??= "#00FF0066", this.attr ??= [], this.piece = null, this.isSelected = !1, this.clearTarget();
  }
  /** マス目の選択状態
   * @param {boolean} value
   */
...
function $(p, e, t, a) {
  ...
  function f(m, [h, { isAttack: B }], { oX: W, oY: w, isOwn: x }) {
    if (x)
      for (const [X, { child: b = [] } = {}] of Be)
        for (let E = 0; E < m.length; E++)
          for (let v = 0; v < m[E].length; v++) {
            const [O, L] = [v + t - W, E + a - w];
            !o(O, L) || !c(B, O, L, h) || m[E][v] !== X || S(m, b, !1, W, w) || u(h, O, L);
          }
  }
...
const Ce = Object.keys(C.degChars), ee = () => ({
  panel: null,
  piece: null
});
class we {
  constructor() {
    this.degs = {}, Ce.forEach((e) => this.degs[e] = ee());
  }
  /** アンパッサン情報をクリア
...
class Z {
  ...
  constructor(e, t) {
    const {
      ...
      onGameOver: I = (g, k) => alert(`プレイヤー${k + 1}の敗北です。`),
    ...
    let H = null, G = null;
    if (b || (H = P.importAsync(), G = D.importAsync(), this.canvas = e, this.ctx = e.getContext("2d"), this.ctx.clearRect(0, 0, e.width, e.height), this.overlay = new be(this.canvas, v), this.#i = new ue()), this.pieces = C.getPieces(this.ctx, {
      size: B,
      useRankSize: W,
      isDrawShadow: w
    ...
    this.playerLen = l, this.left = f, this.top = A, this.panelWidth = m, this.panelHeight = h, this.borderWidth = x, this.pieceSize = B, this.canvasBackgroundColor = X, this.field = this.field.map(
      (g, k) => [...g].map((N, M) => {
        const T = f + m * (M + 1), Y = A + h * (k + 1);
        return new he(this.ctx, N, T, Y, m, h, M, k, x);
      })
    ...
    this.autoDrawing = E, E && (H.then(() => {
      this.draw(), this.#i.setFontFamily(P.names);
    }), G.then(() => this.draw()), this.draw()), this.isGameEnd = !1, this.onDrawed = j, this.onTurnEnd = F, this.onGameOver = I, this.onGameEnd = R, this.moveMode = O, this.record = [], this.turn = 0, b || (this.#e = Ae(this)), L && (this.#t = this.makePlayerControl(), this.#t.add()), this.enPassant = new we();
  }
  /** 操作パネルを構築
   * @param {string[]} compList - 表示するコントロールの一覧
...
class ke extends J {
  ...
  constructor(e, t) {
    ...
    this.engine = a == null ? null : new K[a](e, t);
  }
  /** 手番操作 */
  async playTurn() {
...
function Ee(p) {
  Object.assign(P, p.canvasFont), Object.assign(oe, p.gameSoft), Object.assign(Q, p.games), Object.assign(q, p.boards), Object.assign(z, p.panels), Object.assign(U, p.pieces), Object.assign(te, p.pieceRange), Object.assign(V, p.pieceCost);
}
Ee(le);
const ve = () => [
  .../* @__PURE__ */ new Set([
    ...ne.buttonTexts + Object.values(z).map(({ displayText: p }) => p).join("") + Object.values(U).map(({ display: p }) => p ? p.join("") : "").join("")
  ])
].sort().join("");
Object.assign(P, {
  /** 読み込み済みであるか? */
