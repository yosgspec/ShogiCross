...
class C {
  ...
  * @type {Object<string, number>}
  ...
  drawImage() {
    if (!this.ctx) return;
    const { ctx: e, size: t, deg: a } = this, n = this.imgSrc[a][this.displayPtn] ?? this.imgSrc[0][this.displayPtn], s = G.images[n];
    if (!s) return;
...
const fe = Object.keys(C.degChars), te = () => ({ pX: null, pY: null, pieceId: null });
class q {
  constructor() {
    this.degs = {}, fe.forEach((e) => this.degs[e] = te());
  }
  /** アンパッサン情報をクリア
...
class ee {
  ...
  constructor(e, t) {
    const {
      ...
      onGameOver: I = (w, v) => alert(`プレイヤー${v + 1}の敗北です。`),
      onGameEnd: T = (w, v) => w.record.add({ end: `対戦終了 勝者${[...w.players.values()][v].degChar}` })
    } = t;
    ...
    this.playerLen = l, this.left = f, this.top = y, this.panelWidth = m, this.panelHeight = h, this.borderWidth = k, this.pieceSize = B, this.canvasBackgroundColor = X, this.field = this.field.map(
      (w, v) => [...w].map((u, x) => {
        ...
        return new ie(this.ctx, u, N, L, m, h, x, v, k);
      })
    ...
    for (let w = 0; w < l; w++) {
      ...
      if (u.cpu = new pe(this, u), this.players.set(v, u), !!u.gameName)
        try {
    ...
    this.width = this.panelWidth * (this.xLen + 1), this.height = this.panelHeight * (this.yLen + 1), this.right = f + this.width, this.bottom = y + this.height, this.stand = new K(this), this.isGameEnd = !1, this.onDrawed = $, this.onTurnEnd = F, this.onGameOver = I, this.onGameEnd = T, this.moveMode = M, this.record = new ne(this), this.enPassant = new q();
  }
  /** ゲームを実行する
   * @param {HTMLCanvasElement}} canvas - Canvas要素
   * @param {BoardInitOption} option - ボードの初期化オプション
...
async function We(p, e = "image", t = "png", a = "base64") {
  const n = Be(t), s = document.createElement("a");
  let i;
...
function D(p, e, t, a) {
  ...
  function y(m, [h, { isAttack: B }], { oX: W, oY: b, isOwn: k, offsetX: X, offsetY: E }) {
    if (!(!k && !c(!1, t + X, a + E)))
      for (const [O, { jmps: P = 0, moves: M = 0 } = {}] of re) {
        const R = !M || M === 0;
        for (let $ = b - 1; $ <= b + 1; $++)
          for (let F = W - 1; F <= W + 1; F++) {
            ...
            for (let u = t, x = a; ; ) {
              u += w, x += v;
              const N = u + X, L = x + E;
              if (!o(N, L) || !R && T === 0) break;
              const H = I === 0;
              H && c(B, N, L, h, H) ? (T--, g(h, N, L)) : P < 1 && T--;
              const z = s[L][N];
              if (z.piece && (I--, H || l(z)))
                break;
            }
  ...
  const { canvas: s } = p, i = async (c, S, g = () => {
    ...
    e = !0, await i(
      ...
      (S, g, f) => {
        ...
        y && S.checkRangeMouse(g, f) && (c.preventDefault(), y.isSelected = !0, a = S, D(p, y, m, h));
      },
      (S, g) => {
...
class ce extends ee {
  ...
  constructor(e, t) {
    ...
    const {
      ...
      onGameOver: I = (u, x) => alert(`プレイヤー${x + 1}の敗北です。`),
      onGameEnd: T = (u, x) => u.record.add({ end: `対戦終了 勝者${[...u.players.values()][x].degChar}` })
    } = t;
    let w = null, v = null;
    if (E || (w = j.importAsync(), v = G.importAsync(), this.canvas = e, this.ctx = e.getContext("2d"), this.ctx.clearRect(0, 0, e.width, e.height), this.overlay = new Ne(this.canvas, P), this.#a = new Ae()), this.pieces = C.getPieces(this.ctx, {
      size: B,
      useRankSize: W,
      isDrawShadow: b
    }), !Y[r]) throw Error(`playBoard=${r}, Unknown board name.`);
    ...
    this.playerLen = l, this.left = f, this.top = y, this.panelWidth = m, this.panelHeight = h, this.borderWidth = k, this.pieceSize = B, this.canvasBackgroundColor = X, this.field = this.field.map(
      (u, x) => [...u].map((N, L) => {
        const H = f + m * (L + 1), z = y + h * (x + 1);
        return new ie(this.ctx, N, H, z, m, h, L, x, k);
      })
    ), this.xLen = this.field[0].length, this.yLen = this.field.length, this.players = /* @__PURE__ */ new Map();
    for (let u = 0; u < l; u++) {
      ...
      if (N.cpu = new pe(this, N), this.players.set(x, N), !!N.gameName)
        try {
    ...
    if (this.width = this.panelWidth * (this.xLen + 1), this.height = this.panelHeight * (this.yLen + 1), this.right = f + this.width, this.bottom = y + this.height, this.stand = new K(this), !E) {
    ...
    }), v.then(() => this.draw()), this.draw()), this.isGameEnd = !1, this.onDrawed = $, this.onTurnEnd = F, this.onGameOver = I, this.onGameEnd = T, this.moveMode = M, this.record = new ne(this), E || (this.#e = Ee(this)), R && (this.#t = this.makePlayerControl(), this.#t.add()), this.enPassant = new q();
  }
  /** 操作パネルを構築
   * @param {string[]} compList - 表示するコントロールの一覧
   * @returns {PlayerControl}
   */
  makePlayerControl(e) {
    return this.#t = new se(this, e), this.#t;
  }
  /** ボードを閉じる */
  close() {
  ...
  async playTurn() {
    ...
    e.field.flat().forEach((i) => {
      if (i.piece && i.piece.deg === t.deg) {
        const r = i, o = D(e, r.piece, r.pX, r.pY);
  ...
  async playTurn() {
    ...
    e.field.flat().forEach((o) => {
      if (o.piece && o.piece.deg === t.deg) {
        const l = o, d = D(e, l.piece, l.pX, l.pY);
  ...
  async minimax(e, t, a, n, s) {
    ...
    e.field.flat().forEach((l) => {
      if (l.piece && l.piece.deg === i.deg) {
        const d = l, c = D(e, d.piece, d.pX, d.pY);
...
class pe extends V {
  ...
  constructor(e, t) {
    ...
    this.engine = a == null ? null : new Z[a](e, t);
  }
  /** 手番操作 */
  async playTurn() {
...
function Xe(p) {
  Object.assign(j, p.canvasFont), Object.assign(Se, p.gameSoft), Object.assign(_, p.games), Object.assign(Y, p.boards), Object.assign(U, p.panels), Object.assign(J, p.pieces), Object.assign(ae, p.pieceRange), Object.assign(Q, p.pieceCost);
}
Xe(me);
const Oe = () => [
  .../* @__PURE__ */ new Set([
    ...se.buttonTexts + Object.values(U).map(({ displayText: p }) => p).join("") + Object.values(J).map(({ display: p }) => p ? p.join("") : "").join("")
  ])
].sort().join("");
Object.assign(j, {
  /** 読み込み済みであるか? */
  ...
  async importAsync() {
    ...
    const p = "https://fonts.googleapis.com/css2?family=", e = Oe(), t = Date.now().toString();
